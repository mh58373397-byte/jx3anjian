...
1. 修复了宏无法录制鼠标侧键的问题
2. 更新复选框，勾选后按键仅在游戏内生效
3. 更新了按键快速闪烁复选框的功能
4. 修复了系统功能键无法作为宏快捷键的问题
5. 优化了F9游戏模式的显示，新增"丐意值"参数，可以调节特效炫酷程度

### v5.0 新增与优化（补充）

- **多配置文件管理**：支持同目录 `*.json` 配置下拉切换，支持一键新建配置（如 `config1.json`），支持内联重命名（下拉切输入框，`Enter` 确认、`Esc` 取消）
- **配置兜底生成逻辑**：当 `config.json` 缺失时，会优先按当前选中的配置内容生成新的 `config.json`
- **模式切换 UI 修复**：修复切换不同配置后模式单选框显示异常（可能出现多选）的 UI 同步问题
- **仅在当前窗口生效**：新增确认提示，开启后连发仅在启动时记录的窗口中生效，切出窗口自动暂停，切回自动恢复
- **技能快速闪烁增强**：新增确认提示说明性能影响；开启后连发按键发送顺序切换为“弹起→按下→延迟”，未开启为“按下→弹起→延迟”
- **游戏模式增强**：新增霓虹风格显示与动态特效，右键菜单增加“丐意值”滑块用于调节特效强弱（0% 强，100% 弱）
- **游戏模式体验修复**：移除切换横幅、移除底部 PPS 条、修复主界面返回后部分标签偶发丢字、修复丐意值弹窗焦点问题，并保持固定宽高比缩放体验
