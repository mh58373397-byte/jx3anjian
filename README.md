...
1. 修复了功能区（Ctrl、Alt 等）按键经过调换后，无法正常连发的问题
2. 修复了某些情况下，全局停止时宏不停止的问题
3. 修复了宏可能会以按下状态结束的问题
4. 修复了宏录制无法记录鼠标滚轮的问题
5. 修复了勾选复选框后，宏在按压模式下无法打断 Toggle 按键的问题
6. 新增独占模式
7. 新增 WASD 稳定输入模式（WASD 按下时，连发与宏循环自动降速至稳定节奏）
...
- **独立开关** — 可选择"不开启连发键位对调也生效"，让对调功能独立于连发工作
...
- **WASD 稳定输入模式** — 勾选后，检测到 WASD 按下时自动将连发节奏下限提升到 30ms，并在宏每轮循环末尾额外增加 30ms，WASD 全部弹起后自动恢复
...
勾选 **「不开启连发键位对调也生效」** 后，键位对调无需开启连发即可工作。

### 独占模式

#### 1) 如何设置独占

1. 在主界面虚拟键盘上，用**鼠标右键**点击目标按键。
2. 在弹出菜单中选择 **「设为独占模式」** 或 **「取消独占模式」**。
3. 设置后按键右上角会显示 **E** 角标，图例显示为 **独占(鼠标右键设置)**。

#### 2) 独占的核心行为

- **独占 Hold（非排除）**：按住该键时会暂时打断其它 Toggle 连发，松开后其它 Toggle 自动恢复。
- **独占 Toggle**：只要有任一独占 Toggle 处于开启状态，其它非独占 Toggle 自动暂停；全部独占 Toggle 关闭后自动恢复。
- **排除键 + 独占**：按下时等效“暂停模式下全局停止”（无声音），松开时等效“暂停模式下全局启动”（无声音）。
- **宏键 + 独占**：非按压模式下，独占宏键触发宏运行时会打断正在运行的 Toggle；宏结束后 Toggle 自动恢复。

#### 3) 与键位对调的关系

独占判定跟随**对调后的逻辑功能键**，不是物理键位置。  
例如把 F 设为独占再与 G 对调后，物理按 G（实际执行 F 功能）会继承 F 的独占行为。

#### 4) 约束与冲突规则

- 全局启停热键和游戏模式热键不能设为独占。
- 若某个独占键随后被设置为全局启停热键或游戏模式热键，会立即自动取消独占。
- 独占配置会写入当前配置文件并随配置切换保存/加载。
...
勾选 **「WASD 输入稳定模式」** 后：
- 只要检测到 `W/A/S/D` 任一键按下，Hold/Toggle/Hybrid 的连发间隔会临时限制为不低于 **30ms**
- 同时，宏回放（按压模式与非按压模式）在每轮循环结束后会额外等待 **30ms**
- 当 `W/A/S/D` 全部弹起后，连发与宏回放自动恢复原设定速率
